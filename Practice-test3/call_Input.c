...
int last_popStack(char operator[10], int post[10], int index, int i){
    // checking operator 
    int result;
    if (operator[index] == '*' )
    {
        result =  (post[i] * post[i-1]);
    }
    else if(operator[index] == '/'){ 
        result =  (post[i] / post[i-1]);
    }
...
int main() { 
                ...
                // ในกรณีน้อยกว่าหรือเท่ากับ
                    ...
                    while (compareOprator(stack[nstack-1], num[i])==2 && strlen(stack)!=0)
                    {
                        if (npost > 1)
                        {
                            ...
                            post[npost-1] = last_popStack(stack, post, nstack-1, npost);
                            npost = npost - 1;
                            stack[nstack-1] = '\0';
                            nstack--;
                        }
    ...
    if (strlen(stack)!=0)
    {
        for ( i = 0; i <= strlen(stack); i++)
        {
            if (npost==1)
            {
                post[nstack-1] = last_popStack(stack, post, nstack-1, npost);
            }
            else { 
                post[npost - 2] += popStack(stack, post, nstack-1, npost);
